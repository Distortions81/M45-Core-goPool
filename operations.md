...
- `optimistic_share_response` (default `true`) – sends the "accepted" response immediately (using a fast-path that extracts the JSON-RPC id and avoids full JSON parsing), then validates async. This dramatically reduces displayed ping (~sub-ms to a few ms, depending on host load) but means invalid shares get "accepted" feedback before being silently rejected. Safe for solo pools. Set to `false` if you prefer responses only after full validation.
